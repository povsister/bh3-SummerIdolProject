...
func (r *RitaRossweisse) RoundAttack(round uint16) {
	...
	if r.Rand(35) {
		r.Rival.DirectTakeDamage(round, r.Rival.Attributes().trueDamage(r.Attack)-3, Normal)
		r.Rival.Attributes().Attack -= 4
		if r.Rival.Attributes().Attack < 0 {
			r.Rival.Attributes().Attack = 0
		}
	} else {
		r.Rival.TakeDamage(round, r.Attack, Normal)
	}
}

func (r *RitaRossweisse) TakeDamage(round uint16, damage int16, form AttackType) {
	switch round % 4 {
	case 1, 2:
		if form == Unique {
...
func (r *RitaRossweisse) DirectTakeDamage(round uint16, damage int16, form AttackType) {
	switch round % 4 {
	case 1, 2:
		if form == Unique {
			// skill damage do not take effect
